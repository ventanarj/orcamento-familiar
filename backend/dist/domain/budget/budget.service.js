...
class BudgetService {
    async getBudgetCycle(householdId) {
        return database_1.database.budgetCycle.findFirst({
            where: { householdId },
            include: {
                allocations: {
    ...
    async getBudgetForecast(householdId) {
        const cycle = await database_1.database.budgetCycle.findFirst({
            where: { householdId },
            include: {
                allocations: {
                    include: { weeklyAllocations: true, category: true },
        ...
        const allocations = cycle.allocations.map((allocation) => {
    ...
    buildRedistribution(sources, sinks) {
    ...
    async calculateWeeklyDistribution(budgetCycleId) {
        const cycle = await database_1.database.budgetCycle.findUnique({
        ...
        return cycle.allocations.map((allocation) => {
